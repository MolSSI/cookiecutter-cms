...
# Modified by Levi Naden to handle packages on Pip as needed
...
function main () {
    ...
    InstallCondaPackages $env:PYTHON "conda-build pip jinja2 anaconda-client pip pytest pytest-cov"
    {% if cookiecutter.dependency_source == "conda-forge" %}
    InstallCondaPackages $env:PYTHON "codecov"
    {% else %}
    InstallPipPackages $env:PYTHON codecov
    {% endif %}
